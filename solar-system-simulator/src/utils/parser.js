...
export function parseDistance(str) {
  if (str.includes("M KM")) {
    ...
    return match ? parseFloat(match[1]) * 149_597_870.7 : null;
  } else if (str.includes("KM")) {
    const match = str.match(/([\d\s]+)\s*KM/i);
    if (match) {
      const normalized = match[1].replace(/\s/g, ""); // Remove all spaces
...
export const KM_PER_SCENE_UNIT = 10_000;   
